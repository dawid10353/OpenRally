...
- **Środowisko**: Użytkownik przechodzi na WSL (Linux) — używaj standardowych komend Linux (npm, npx, node). 
  - **⚠️ WAŻNA INSTRUKCJA DLA AI ⚠️**: Terminal wbudowany w Twoim środowisku to Windows PowerShell. Domyślna polityka wykonywania skryptów w systemie Windows blokuje bezpośrednie użycie komend takich jak `npm` (np. wywołanie `npm.ps1`), co powoduje błąd `UnauthorizedAccess`. Zawsze uruchamiaj narzędzia node/npm z prefiksem `cmd.exe /c` (np. `cmd.exe /c npm run lint`, `cmd.exe /c npm install`) aby obejść tę restrykcję w PowerShellu.
---
